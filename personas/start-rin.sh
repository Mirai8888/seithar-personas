...
#   PERSONA_LOCAL_MODEL=<name>   (default: qwen2.5:14b)
#   PERSONA_LOCAL_TIMEOUT=45
#
# What this starts:
#   1. Rin Discord bot (engine.py --persona rin) — local inference only
#   2. Rin engagement loop (tweets + send DMs from DB) — LLM for tweets + DM text only
# Optional: RIN_START_SCRAPER=1 — also start background scraper (no LLM) to fill DMable DB
#
# Logs:
...
export PERSONA_LOCAL_MODEL="${PERSONA_LOCAL_MODEL:-qwen2.5:14b}"

echo "[personas] Rin only, local inference (PERSONA_LOCAL_MODEL=$PERSONA_LOCAL_MODEL)..."
